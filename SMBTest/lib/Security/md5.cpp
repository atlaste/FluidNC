...
namespace Security {
     ...
     * of bytes.
     */
    void MD5::Update(unsigned char* buf, unsigned len)
    {
        uint32_t t;
        ...
        Transform(reinterpret_cast<uint32_t*>(buf), in);
        buf += t;
        len -= t;
    ...
    {
        int count = bytes[0] & 0x3f;	/* Number of bytes in in */
 ...
 */
    void MD5::Transform(uint32_t* buf, uint32_t const* in)
    {
