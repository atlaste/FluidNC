...
int smb3_encrypt_pdu(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    for (tmp_pdu = pdu; tmp_pdu; tmp_pdu = tmp_pdu->next_compound) {
        for (i = 0; i < tmp_pdu->out.niov; i++) {
            memcpy(&pdu->crypt[spl], tmp_pdu->out.iov[i].buf, tmp_pdu->out.iov[i].len);
            spl += tmp_pdu->out.iov[i].len;
        }
    }

    aes128ccm_encrypt(smb2->serverin_key, &pdu->crypt[20], 11, &pdu->crypt[20], 32, &pdu->crypt[52], spl - 52, &pdu->crypt[4], 16);
    pdu->crypt_len = spl;
...
int smb3_decrypt_pdu(struct smb2_context* smb2) {
    ...
    if (aes128ccm_decrypt(smb2->serverout_key,
                          &smb2->in.iov[smb2->in.niov - 2].buf[20],
                          11,
                          &smb2->in.iov[smb2->in.niov - 2].buf[20],
                          32,
                          &smb2->in.iov[smb2->in.niov - 1].buf[0],
                          smb2->in.iov[smb2->in.niov - 1].len,
                          &smb2->in.iov[smb2->in.niov - 2].buf[4],
                          16)) {
