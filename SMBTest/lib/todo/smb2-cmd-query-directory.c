...
int smb2_decode_fileidfulldirectoryinformation(struct smb2_context*                        smb2,
    ...
    if (name_len > 80 + name_len || 80 + name_len > vec->len) {
...
static int smb2_encode_query_directory_request(struct smb2_context* smb2, struct smb2_pdu* pdu, struct smb2_query_directory_request* req) {
    int                len;
    uint8_t*           buf;
...
#define IOV_OFFSET (rep->output_buffer_offset - SMB2_HEADER_SIZE - (SMB2_QUERY_DIRECTORY_REPLY_SIZE & 0xfffe))

int smb2_process_query_directory_fixed(struct smb2_context* smb2, struct smb2_pdu* pdu) {
...
int smb2_process_query_directory_variable(struct smb2_context* smb2, struct smb2_pdu* pdu) {
