...
int smb2_pad_to_64bit(struct smb2_context* smb2, struct smb2_io_vectors* v) {
    ...
    if (smb2_add_iovector(smb2, v, &zero_bytes[0], 8 - (len & 0x07), NULL) == NULL) {
        return -1;
    }
...
static void smb2_encode_header(struct smb2_context* smb2, struct smb2_iovec* iov, struct smb2_header* hdr) {
...
int smb2_decode_header(struct smb2_context* smb2, struct smb2_iovec* iov, struct smb2_header* hdr) {
...
static void smb2_add_to_outqueue(struct smb2_context* smb2, struct smb2_pdu* pdu) {
...
void smb2_queue_pdu(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    for (p = pdu; p; p = p->next_compound) {
        ...
        if (smb2->sign || (p->header.command == SMB2_TREE_CONNECT && smb2->dialect == SMB2_VERSION_0311 && !smb2->seal)) {
...
static int smb2_is_error_response(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    if ((smb2->hdr.status & SMB2_STATUS_SEVERITY_MASK) == SMB2_STATUS_SEVERITY_ERROR) {
    ...
    } else if ((smb2->hdr.status & SMB2_STATUS_SEVERITY_MASK) == SMB2_STATUS_SEVERITY_WARNING) {
        switch (smb2->hdr.status) {
...
void smb2_timeout_pdus(struct smb2_context* smb2) {
    ...
    while (pdu) {
        ...
        if (pdu->timeout && pdu->timeout < t) {
            ...
            pdu->cb(smb2, SMB2_STATUS_IO_TIMEOUT, NULL, pdu->cb_data);
