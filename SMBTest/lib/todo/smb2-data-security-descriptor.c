...
static struct smb2_sid* decode_sid(struct smb2_context* smb2, void* memctx, struct smb2_iovec* v) {
    struct smb2_sid* sid;
    ...
    sid = smb2_alloc_data(smb2, memctx, offsetof(struct smb2_sid, sub_auth) + sub_auth_count * sizeof(uint32_t));
    if (sid == NULL) {
    ...
    for (i = 0; i < sub_auth_count; i++) {
        smb2_get_uint32(v, 8 + i * sizeof(uint32_t), &sid->sub_auth[i]);
    }
...
static struct smb2_ace* decode_ace(struct smb2_context* smb2, void* memctx, struct smb2_iovec* vec) {
...
static struct smb2_acl* decode_acl(struct smb2_context* smb2, void* memctx, struct smb2_iovec* vec) {
    struct smb2_iovec v = *vec;
    struct smb2_acl*  acl;
    uint8_t           revision;
    ...
    for (i = 0; i < ace_count; i++) {
        ...
        if (ace == NULL) {
            smb2_set_error(smb2, "failed to decode ace # %d: %s", i, smb2_get_error(smb2));
            return NULL;
        }
        /* skip to the next ace */
        if (ace->ace_size > v.len) {
            smb2_set_error(smb2, "not enough data for ace %s", smb2_get_error(smb2));
            return NULL;
        }
        v.len -= ace->ace_size;
...
int smb2_decode_security_descriptor(struct smb2_context* smb2, void* memctx, struct smb2_security_descriptor* sd, struct smb2_iovec* vec) {
    struct smb2_iovec v;
    ...
    if (sd->revision != 1) {
        smb2_set_error(smb2,
                       "can not decode security descriptor with "
                       "revision %d",
                       sd->revision);
        return -1;
    }
    ...
    if (offset_owner > 0 && offset_owner + 2 + SID_ID_AUTH_LEN < vec->len) {
        ...
        if (sd->owner == NULL) {
            smb2_set_error(smb2, "failed to decode owner sid: %s", smb2_get_error(smb2));
            return -1;
        }
    }
    ...
    if (offset_group > 0 && offset_group + 2 + SID_ID_AUTH_LEN < vec->len) {
        ...
        if (sd->group == NULL) {
            smb2_set_error(smb2, "failed to decode group sid: %s", smb2_get_error(smb2));
            return -1;
        }
    }
    ...
    if (offset_dacl > 0 && offset_dacl + 8 <= vec->len) {
        ...
        if (sd->dacl == NULL) {
            smb2_set_error(smb2, "failed to decode dacl: %s", smb2_get_error(smb2));
            return -1;
        }
    }
