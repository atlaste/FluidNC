...
static int smb2_encode_tree_connect_request(struct smb2_context* smb2, struct smb2_pdu* pdu, struct smb2_tree_connect_request* req) {
    int                len;
    uint8_t*           buf;
    ...
    if (buf == NULL) {
        smb2_set_error(smb2,
                       "Failed to allocate tree connect setup "
                       "buffer");
        return -1;
    }
    ...
    iov = smb2_add_iovector(smb2, &pdu->out, buf, req->path_length, free);
...
int smb2_process_tree_connect_fixed(struct smb2_context* smb2, struct smb2_pdu* pdu) {
