...
#include "../include/portable-endian.h"
#include "ntlmssp.h"
...
const smb2_file_id compound_file_id = { 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff };
...
static void smb2_close_context(struct smb2_context* smb2) {
    if (smb2 == NULL) {
        return;
    }
...
static int send_session_setup_request(struct smb2_context* smb2, struct connect_data* c_data, unsigned char* buf, int len);
...
static void od_close_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void query_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    ...
    smb2_set_error(smb2, "Query directory failed with (0x%08x) %s. %s", status, nterror_to_str(status), smb2_get_error(smb2));
    dir->cb(smb2, -nterror_to_errno(status), NULL, dir->cb_data);
    free_smb2dir(smb2, dir);
}

static void opendir_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct smb2dir*                     dir = private_data;
    ...
    if (status != SMB2_STATUS_SUCCESS) {
        smb2_set_error(smb2, "Opendir failed with (0x%08x) %s.", status, nterror_to_str(status));
        dir->cb(smb2, -nterror_to_errno(status), NULL, dir->cb_data);
        free_smb2dir(smb2, dir);
        return;
    }
...
int smb2_opendir_async(struct smb2_context* smb2, const char* path, smb2_command_cb cb, void* cb_data) {
...
static void free_c_data(struct smb2_context* smb2, struct connect_data* c_data) {
    if (c_data->auth_data) {
...
static void tree_connect_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct connect_data* c_data = private_data;

    if (status != SMB2_STATUS_SUCCESS) {
        smb2_close_context(smb2);
        smb2_set_error(smb2, "Tree Connect failed with (0x%08x) %s. %s", status, nterror_to_str(status), smb2_get_error(smb2));
...
/* MS-SMB2 3.2.5.2 */
static int smb3_update_preauth_hash(struct smb2_context* smb2, int niov, struct smb2_iovec* iov) {
    int         i;
    USHAContext tctx;
...
static void session_setup_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct connect_data*             c_data = private_data;
    ...
    if (status == SMB2_STATUS_MORE_PROCESSING_REQUIRED && rep->security_buffer) {
        ...
        if ((ret = send_session_setup_request(smb2, c_data, rep->security_buffer, rep->security_buffer_length)) < 0) {
            smb2_close_context(smb2);
        ...
        smb2_close_context(smb2);
        smb2_set_error(smb2, "Session setup failed with (0x%08x) %s", status, nterror_to_str(status));
        c_data->cb(smb2, -nterror_to_errno(status), NULL, c_data->cb_data);
        free_c_data(smb2, c_data);
        return;
    }
    ...
    if (smb2->sec == SMB2_SEC_KRB5) {
        /* For NTLM the status will be
                 ...
                 * ignore krb5 errors.
                 */
        krb5_session_request(smb2, c_data->auth_data, rep->security_buffer, rep->security_buffer_length);
    }
#endif
    ...
    if (smb2->sign || smb2->seal || smb2->dialect == SMB2_VERSION_0311) {
        ...
        if (smb2->sec == SMB2_SEC_NTLMSSP) {
            if (ntlmssp_get_session_key(c_data->auth_data, &smb2->session_key, &smb2->session_key_size) < 0) {
        ...
        if (smb2->dialect == SMB2_VERSION_0202 || smb2->dialect == SMB2_VERSION_0210) {
            ...
            smb2_derive_key(
                smb2->session_key, smb2->session_key_size, SMB2AESCMAC, sizeof(SMB2AESCMAC), SmbSign, sizeof(SmbSign), smb2->signing_key);
            smb2_derive_key(
                smb2->session_key, smb2->session_key_size, SMB2AESCCM, sizeof(SMB2AESCCM), ServerIn, sizeof(ServerIn), smb2->serverin_key);
            smb2_derive_key(
                smb2->session_key, smb2->session_key_size, SMB2AESCCM, sizeof(SMB2AESCCM), ServerOut, sizeof(ServerOut), smb2->serverout_key);
        } else if (smb2->dialect > SMB2_VERSION_0302) {
        ...
        if (smb2->hdr.flags & SMB2_FLAGS_SIGNED) {
            ...
            if (smb2_calc_signature(smb2, &smb2->in.iov[1].buf[48], &smb2->in.iov[1], smb2->in.niov - 1) < 0) {
...
static int send_session_setup_request(struct smb2_context* smb2, struct connect_data* c_data, unsigned char* buf, int len) {
    ...
    if (smb2->sec == SMB2_SEC_NTLMSSP) {
        if (ntlmssp_generate_blob(smb2, time(NULL), c_data->auth_data, buf, len, &req.security_buffer, &req.security_buffer_length) < 0) {
            smb2_close_context(smb2);
            return -1;
        }
    }
...
static void negotiate_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct connect_data*         c_data = private_data;
    ...
    if (status != SMB2_STATUS_SUCCESS) {
        smb2_close_context(smb2);
        smb2_set_error(smb2, "Negotiate failed with (0x%08x) %s. %s", status, nterror_to_str(status), smb2_get_error(smb2));
        c_data->cb(smb2, -nterror_to_errno(status), NULL, c_data->cb_data);
        free_c_data(smb2, c_data);
        return;
    }
    ...
    if (smb2->seal && (smb2->dialect == SMB2_VERSION_0300 || smb2->dialect == SMB2_VERSION_0302)) {
    ...
    if (smb2->sec == SMB2_SEC_NTLMSSP) {
        ...
        c_data->auth_data = krb5_negotiate_reply(smb2, c_data->server, smb2->domain, c_data->user, smb2->password);
    }
#endif
...
static void connect_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct connect_data*          c_data = private_data;
    ...
    if (smb2->version == SMB2_VERSION_ANY || smb2->version == SMB2_VERSION_ANY3 || smb2->version == SMB2_VERSION_0300 ||
        smb2->version == SMB2_VERSION_0302 || smb2->version == SMB2_VERSION_0311) {
...
int smb2_connect_share_async(
    struct smb2_context* smb2, const char* server, const char* share, const char* user, smb2_command_cb cb, void* cb_data) {
...
static void open_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    ...
    if (status != SMB2_STATUS_SUCCESS) {
        smb2_set_error(smb2, "Open failed with (0x%08x) %s.", status, nterror_to_str(status));
...
static void close_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    ...
    if (status != SMB2_STATUS_SUCCESS) {
        smb2_set_error(smb2, "Close failed with (0x%08x) %s", status, nterror_to_str(status));
        fh->cb(smb2, -nterror_to_errno(status), NULL, fh->cb_data);
        free_smb2fh(smb2, fh);
        return;
    }
...
static void fsync_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    ...
    if (status != SMB2_STATUS_SUCCESS) {
        smb2_set_error(smb2, "Flush failed with (0x%08x) %s", status, nterror_to_str(status));
        fh->cb(smb2, -nterror_to_errno(status), NULL, fh->cb_data);
        return;
    }
...
static void read_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    ...
    if (status && status != SMB2_STATUS_END_OF_FILE) {
        smb2_set_error(smb2, "Read/Write failed with (0x%08x) %s", status, nterror_to_str(status));
...
int smb2_pread_async(
    struct smb2_context* smb2, struct smb2fh* fh, uint8_t* buf, uint32_t count, uint64_t offset, smb2_command_cb cb, void* cb_data) {
...
int smb2_pwrite_async(
    struct smb2_context* smb2, struct smb2fh* fh, const uint8_t* buf, uint32_t count, uint64_t offset, smb2_command_cb cb, void* cb_data) {
...
int smb2_write_async(struct smb2_context* smb2, struct smb2fh* fh, const uint8_t* buf, uint32_t count, smb2_command_cb cb, void* cb_data) {
    if (smb2 == NULL) {
        return -EINVAL;
    }
    if (fh == NULL) {
...
static void create_cb_2(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void create_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct create_cb_data*    create_data = private_data;
    struct smb2_create_reply* rep         = command_data;
    struct smb2_close_request req;
    struct smb2_pdu*          pdu;
...
static int smb2_unlink_internal(struct smb2_context* smb2, const char* path, int is_dir, smb2_command_cb cb, void* cb_data) {
    ...
    req.share_access       = SMB2_FILE_SHARE_READ | SMB2_FILE_SHARE_WRITE | SMB2_FILE_SHARE_DELETE;
    req.create_disposition = SMB2_FILE_OPEN;
...
static void fstat_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void getinfo_cb_3(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct stat_cb_data* stat_data = private_data;
    ...
    stat_data->cb(smb2, -nterror_to_errno(stat_data->status), stat_data->st, stat_data->cb_data);
    free(stat_data);
}

static void getinfo_cb_2(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct stat_cb_data*          stat_data = private_data;
    struct smb2_query_info_reply* rep       = command_data;
    ...
    if (stat_data->info_type == SMB2_0_INFO_FILE && stat_data->file_info_class == SMB2_FILE_ALL_INFORMATION) {
        struct smb2_stat_64*       st = stat_data->st;
        struct smb2_file_all_info* fs = rep->output_buffer;
        ...
        st->smb2_btime      = fs->basic.creation_time.tv_sec;
        st->smb2_btime_nsec = fs->basic.creation_time.tv_usec * 1000;
    } else if (stat_data->info_type == SMB2_0_INFO_FILESYSTEM && stat_data->file_info_class == SMB2_FILE_FS_FULL_SIZE_INFORMATION) {
        ...
        statvfs->f_bsize = statvfs->f_frsize = vfs->bytes_per_sector * vfs->sectors_per_allocation_unit;
...
static void getinfo_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct stat_cb_data* stat_data = private_data;
...
static int smb2_getinfo_async(
    struct smb2_context* smb2, const char* path, uint8_t info_type, uint8_t file_info_class, void* st, smb2_command_cb cb, void* cb_data) {
    struct stat_cb_data*           stat_data;
...
int smb2_stat_async(struct smb2_context* smb2, const char* path, struct smb2_stat_64* st, smb2_command_cb cb, void* cb_data) {
    return smb2_getinfo_async(smb2, path, SMB2_0_INFO_FILE, SMB2_FILE_ALL_INFORMATION, st, cb, cb_data);
}
...
int smb2_statvfs_async(struct smb2_context* smb2, const char* path, struct smb2_statvfs* statvfs, smb2_command_cb cb, void* cb_data) {
    return smb2_getinfo_async(smb2, path, SMB2_0_INFO_FILESYSTEM, SMB2_FILE_FS_FULL_SIZE_INFORMATION, statvfs, cb, cb_data);
}
...
static void trunc_cb_3(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void trunc_cb_2(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct trunc_cb_data* trunc_data = private_data;
...
static void trunc_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct trunc_cb_data* trunc_data = private_data;
...
static void rename_cb_3(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    ...
    rename_data->cb(smb2, -nterror_to_errno(rename_data->status), NULL, rename_data->cb_data);
    free(rename_data);
}

static void rename_cb_2(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct rename_cb_data* rename_data = private_data;
...
static void rename_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct rename_cb_data* rename_data = private_data;
...
int smb2_rename_async(struct smb2_context* smb2, const char* oldpath, const char* newpath, smb2_command_cb cb, void* cb_data) {
    ...
    next_pdu = smb2_cmd_set_info_async(smb2, &si_req, rename_cb_2, rename_data);
    if (next_pdu == NULL) {
        smb2_set_error(smb2, "Failed to create set command. %s", smb2_get_error(smb2));
        free(rename_data);
        smb2_free_pdu(smb2, pdu);
...
static void ftrunc_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void readlink_cb_3(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void readlink_cb_2(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct readlink_cb_data* cb_data = private_data;
...
static void readlink_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct readlink_cb_data* cb_data = private_data;
...
int smb2_readlink_async(struct smb2_context* smb2, const char* path, smb2_command_cb cb, void* cb_data) {
...
static void disconnect_cb_2(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
static void disconnect_cb_1(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
    struct disconnect_data* dc_data = private_data;
    struct smb2_pdu*        pdu;
...
static void echo_cb(struct smb2_context* smb2, int status, void* command_data, void* private_data) {
...
void smb2_fd_event_callbacks(struct smb2_context* smb2, smb2_change_fd_cb change_fd, smb2_change_events_cb change_events) {
