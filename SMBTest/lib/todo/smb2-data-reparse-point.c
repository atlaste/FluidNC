...
int smb2_decode_reparse_data_buffer(struct smb2_context* smb2, void* memctx, struct smb2_reparse_data_buffer* rp, struct smb2_iovec* vec) {
    ...
    switch (rp->reparse_tag) {
        case SMB2_REPARSE_TAG_SYMLINK:
            ...
            tmp                 = utf16_to_utf8((uint16_t*)(&vec->buf[suboffset + 20]), sublen / 2);
            rp->symlink.subname = smb2_alloc_data(smb2, rp, strlen(tmp) + 1);
            ...
            tmp                   = utf16_to_utf8((uint16_t*)(&vec->buf[printoffset + 20]), printlen / 2);
            rp->symlink.printname = smb2_alloc_data(smb2, rp, strlen(tmp) + 1);
