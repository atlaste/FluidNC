...
static void nse_free(struct smb2nse* nse) {
    free(discard_const(nse->se_req.server));
    free(nse);
}

static void srvsvc_ioctl_cb(struct dcerpc_context* dce, int status, void* command_data, void* cb_data) {
...
static void share_enum_bind_cb(struct dcerpc_context* dce, int status, void* command_data, void* cb_data) {
    struct smb2nse*      nse  = cb_data;
    struct smb2_context* smb2 = dcerpc_get_smb2_context(dce);
...
int smb2_share_enum_async(struct smb2_context* smb2, smb2_command_cb cb, void* cb_data) {
    ...
    rc = dcerpc_connect_context_async(dce, "srvsvc", &srvsvc_interface, share_enum_bind_cb, nse);
    if (rc) {
        nse_free(nse);
        dcerpc_destroy_context(dce);
        return rc;
