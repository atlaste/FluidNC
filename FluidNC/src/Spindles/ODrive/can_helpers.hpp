...
// Copyright (c) ODrive Robotics. Integrated in FluidNC by Stefan de Bruijn, 2025.
// Use of this source code is governed by a MIT license that can be found in the LICENSE file on
// https://github.com/odriverobotics/ODrive/blob/master/LICENSE.md
...
namespace Spindles::ODrive {
    ...
    float can_get_signal_raw(
        const uint8_t* buf, const size_t startBit, const size_t length, const bool isIntel, const float factor, const float offset) {
    ...
    constexpr void can_set_signal_raw(
        uint8_t* buf, const float val, const size_t startBit, const size_t length, const bool isIntel, const float factor, const float offset) {
