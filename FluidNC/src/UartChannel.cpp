...
void UartChannel::init() {
    ...
    if (uart) {
        ...
        log_error("UartChannel: missing uart" << int(_uart_num));
    }
...
void UartChannel::init(Uart* uart) {
    ...
    if (_report_interval_ms) {
        log_info("uart_channel" << int(_uart_num) << " created at report interval: " << int(_report_interval_ms));
    } else {
        log_info("uart_channel" << int(_uart_num) << " created");
    }
