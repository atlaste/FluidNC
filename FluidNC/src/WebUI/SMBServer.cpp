...
namespace WebUI {
    ...
    SMBClient::SMBClient(SMBServer* server, AsyncClient* client)
        ...
        log_debug("SMB: New client connected from " << _tcp->remoteIP().toString().c_str());
    }
    ...
    SMBClient::~SMBClient() {
        ...
        log_debug("SMB: Client disconnected");
    }
    ...
    void SMBClient::parseSmbPackets() {
        ...
        while (_rxBuffer.size() >= 4) {
            ...
            if (_rxBuffer[0] != 0x00) {
                log_error("SMB: Invalid NetBIOS header");
                _disconnected = true;
                return;
            }
    ...
    void SMBClient::sendResponse(struct smb2_header* hdr, uint32_t status, const uint8_t* data, size_t dataLen) {
        ...
        *(uint64_t*)(hdrBytes + 40) = _sessionId;
        ...
        if (_tcp && _tcp->connected()) {
            _tcp->write((const char*)response.data(), response.size());
        }
    }
    ...
    void SMBServer::fileIOWorkerTask(void* param) {
        ...
        while (true) {
            if (xQueueReceive(server->_fileIOQueue, &req, portMAX_DELAY) == pdTRUE && req) {
