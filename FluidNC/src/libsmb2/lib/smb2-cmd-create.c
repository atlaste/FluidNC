...
static int smb2_encode_create_request(struct smb2_context* smb2, struct smb2_pdu* pdu, struct smb2_create_request* req) {
    int                i, len;
    uint8_t*           buf;
    uint16_t           ch;
    ...
    if (name) {
        ...
        iov = smb2_add_iovector(smb2, &pdu->out, buf, 2 * name->len, free);
        /* Convert '/' to '\' */
    ...
    if (name == NULL && !req->create_context_length) {
        ...
        iov = smb2_add_iovector(smb2, &pdu->out, &zero, 1, NULL);
    }

    return 0;
}

struct smb2_pdu* smb2_cmd_create_async(struct smb2_context* smb2, struct smb2_create_request* req, smb2_command_cb cb, void* cb_data) {
    struct smb2_pdu* pdu;
...
#define IOV_OFFSET (rep->create_context_offset - SMB2_HEADER_SIZE - (SMB2_CREATE_REPLY_SIZE & 0xfffe))

int smb2_process_create_fixed(struct smb2_context* smb2, struct smb2_pdu* pdu) {
...
int smb2_process_create_variable(struct smb2_context* smb2, struct smb2_pdu* pdu) {
