...
#define IOV_OFFSET (rep->output_offset - SMB2_HEADER_SIZE - (SMB2_IOCTL_REPLY_SIZE & 0xfffe))

int smb2_process_ioctl_fixed(struct smb2_context* smb2, struct smb2_pdu* pdu) {
...
int smb2_process_ioctl_variable(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    switch (rep->ctl_code) {
        case SMB2_FSCTL_GET_REPARSE_POINT:
            ptr = smb2_alloc_init(smb2, sizeof(struct smb2_reparse_data_buffer));
            if (smb2_decode_reparse_data_buffer(smb2, ptr, ptr, &vec)) {
                smb2_set_error(smb2,
                               "could not decode reparse "
                               "data buffer. %s",
                               smb2_get_error(smb2));
                return -1;
            }
...
static int smb2_encode_ioctl_request(struct smb2_context* smb2, struct smb2_pdu* pdu, struct smb2_ioctl_request* req) {
    int                len;
    uint8_t*           buf;
    ...
    smb2_set_uint32(iov, 24, SMB2_HEADER_SIZE + (SMB2_IOCTL_REQUEST_SIZE & 0xfffffffe));
    ...
    if (req->input_count) {
        iov = smb2_add_iovector(smb2, &pdu->out, req->input, req->input_count, NULL);
    }

    return 0;
}

struct smb2_pdu* smb2_cmd_ioctl_async(struct smb2_context* smb2, struct smb2_ioctl_request* req, smb2_command_cb cb, void* cb_data) {
    struct smb2_pdu* pdu;
