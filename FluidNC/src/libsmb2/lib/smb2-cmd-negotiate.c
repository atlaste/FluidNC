...
static int smb2_encode_preauth_context(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    uint8_t*           buf;
    int                len, i, data_len;
...
static int smb2_encode_encryption_context(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    uint8_t*           buf;
    int                len, data_len;
    struct smb2_iovec* iov;
...
static int smb2_encode_negotiate_request(struct smb2_context* smb2, struct smb2_pdu* pdu, struct smb2_negotiate_request* req) {
    uint8_t*           buf;
    int                i, len;
    struct smb2_iovec* iov;
    ...
    if (smb2->version == SMB2_VERSION_ANY || smb2->version == SMB2_VERSION_ANY3 || smb2->version == SMB2_VERSION_0311) {
    ...
    for (i = 0; i < req->dialect_count; i++) {
        smb2_set_uint16(iov, 36 + i * sizeof(uint16_t), req->dialects[i]);
    }

    return 0;
}

struct smb2_pdu* smb2_cmd_negotiate_async(struct smb2_context* smb2, struct smb2_negotiate_request* req, smb2_command_cb cb, void* cb_data) {
    struct smb2_pdu* pdu;
...
#define IOV_OFFSET (rep->security_buffer_offset - SMB2_HEADER_SIZE - (SMB2_NEGOTIATE_REPLY_SIZE & 0xfffe))

static int smb2_parse_encryption_context(struct smb2_context* smb2, struct smb2_negotiate_reply* rep, struct smb2_iovec* iov, int offset) {
    smb2_get_uint16(iov, offset, &rep->cypher);
    return 0;
}

static int smb2_parse_negotiate_contexts(
    struct smb2_context* smb2, struct smb2_negotiate_reply* rep, struct smb2_iovec* iov, int offset, int count) {
    uint16_t type, len;
    ...
    while (count--) {
        ...
        switch (type) {
            ...
            case SMB2_ENCRYPTION_CAP:
                if (smb2_parse_encryption_context(smb2, rep, iov, offset)) {
                    return -1;
                }
                break;
            default:
                smb2_set_error(smb2,
                               "Unknown negotiate context "
                               "type 0x%04x",
                               type);
                return -1;
        }
...
int smb2_process_negotiate_fixed(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    if (rep->dialect_revision >= SMB2_VERSION_0311) {
        return smb2->spl - SMB2_HEADER_SIZE - (SMB2_NEGOTIATE_REPLY_SIZE & 0xfffe);
    } else {
        return IOV_OFFSET + rep->security_buffer_length;
    }
}

int smb2_process_negotiate_variable(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    offset = rep->negotiate_context_offset - SMB2_HEADER_SIZE - (SMB2_NEGOTIATE_REPLY_SIZE & 0xfffe);

    if (offset < 0 || offset > iov->len) {
        return -1;
    }

    if (smb2_parse_negotiate_contexts(smb2, rep, iov, offset, rep->negotiate_context_count)) {
        return -1;
    }
