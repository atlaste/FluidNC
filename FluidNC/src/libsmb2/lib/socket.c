...
static int smb2_connect_async_next_addr(struct smb2_context* smb2, const struct addrinfo* base);
...
static int smb2_get_credit_charge(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    int credits = 0;
...
int smb2_which_events(struct smb2_context* smb2) {
    ...
    if (smb2->outqueue != NULL && smb2_get_credit_charge(smb2, smb2->outqueue) <= smb2->credits) {
        events |= POLLOUT;
    }
...
const t_socket* smb2_get_fds(struct smb2_context* smb2, size_t* fd_count, int* timeout) {
    if (smb2->fd != -1) {
        *fd_count = 1;
        *timeout  = -1;
        return &smb2->fd;
...
static int smb2_write_to_socket(struct smb2_context* smb2) {
    struct smb2_pdu* pdu;
...
static int smb2_read_data(struct smb2_context* smb2, read_func func, int has_xfrmhdr) {
    struct iovec     iov[SMB2_MAX_VECTORS];
    struct iovec*    tmpiov;
...
read_more_data:
    ...
    switch (smb2->recv_state) {
        case SMB2_RECV_SPL:
            ...
            smb2_add_iovector(smb2, &smb2->in, &smb2->header[0], SMB2_HEADER_SIZE, NULL);
            goto read_more_data;
        case SMB2_RECV_HEADER:
            if (!memcmp(smb2->in.iov[smb2->in.niov - 1].buf, smb3tfrm, 4)) {
                ...
                smb2_add_iovector(smb2, &smb2->in, malloc(len), len, free);
                memcpy(smb2->in.iov[smb2->in.niov - 1].buf, &smb2->in.iov[smb2->in.niov - 2].buf[52], 12);
                smb2->recv_state = SMB2_RECV_TRFM;
                goto read_more_data;
            }
            if (smb2_decode_header(smb2, &smb2->in.iov[smb2->in.niov - 1], &smb2->hdr) != 0) {
                smb2_set_error(smb2,
                               "Failed to decode smb2 "
                               "header: %s",
                               smb2_get_error(smb2));
                return -1;
            }
            ...
            smb2_add_iovector(smb2, &smb2->in, malloc(len & 0xfffe), len & 0xfffe, free);
            goto read_more_data;
        case SMB2_RECV_FIXED:
            len = smb2_process_payload_fixed(smb2, pdu);
            if (len < 0) {
                smb2_set_error(smb2,
                               "Failed to parse fixed part of "
                               "command payload. %s",
                               smb2_get_error(smb2));
                return -1;
            }
            ...
            if (len) {
                for (i = 0; i < pdu->in.niov; i++) {
                    ...
                    smb2_add_iovector(smb2, &smb2->in, pdu->in.iov[i].buf, num, NULL);
                    len -= num;
            ...
            if (smb2->hdr.next_command) {
                len = smb2->hdr.next_command - (SMB2_HEADER_SIZE + smb2->in.num_done - smb2->payload_offset);
            } else {
        ...
        case SMB2_RECV_VARIABLE:
            if (smb2_process_payload_variable(smb2, pdu) < 0) {
                smb2_set_error(smb2,
                               "Failed to parse variable part of "
                               "command payload. %s",
                               smb2_get_error(smb2));
                return -1;
            }
    ...
    /* We don't yet have the signing key until later, once session
         ...
         */
    if (smb2->sign && (smb2->hdr.flags & SMB2_FLAGS_SIGNED) && (smb2->hdr.command != SMB2_SESSION_SETUP)) {
        uint8_t signature[16];
        memcpy(&signature[0], &smb2->in.iov[1 + iov_offset].buf[48], 16);
        if (smb2_calc_signature(smb2, &smb2->in.iov[1 + iov_offset].buf[48], &smb2->in.iov[1 + iov_offset], smb2->in.niov - 1 - iov_offset) <
            0) {
            return -1;
        }
...
static ssize_t smb2_readv_from_socket(struct smb2_context* smb2, const struct iovec* iov, int iovcnt) {
    return readv(smb2->fd, iov, iovcnt);
}

static int smb2_read_from_socket(struct smb2_context* smb2) {
    ...
    if (smb2->in.num_done == 0) {
        ...
        smb2_add_iovector(smb2, &smb2->in, (uint8_t*)&smb2->spl, SMB2_SPL_SIZE, NULL);
    }

    return smb2_read_data(smb2, smb2_readv_from_socket, 0);
}

static ssize_t smb2_readv_from_buf(struct smb2_context* smb2, const struct iovec* iov, int iovcnt) {
    int i, len, count = 0;
...
static void smb2_close_connecting_fd(struct smb2_context* smb2, int fd) {
    size_t i;
    ...
    for (i = 0; i < smb2->connecting_fds_count; ++i) {
        if (fd == smb2->connecting_fds[i]) {
            memmove(&smb2->connecting_fds[i], &smb2->connecting_fds[i + 1], smb2->connecting_fds_count - i - 1);
...
int smb2_service_fd(struct smb2_context* smb2, int fd, int revents) {
    ...
    if (fd == -1) {
        ...
        if (smb2->next_addrinfo != NULL) {
            int err = smb2_connect_async_next_addr(smb2, smb2->next_addrinfo);
            return err == 0 ? 0 : -1;
        }
        goto out;
    } else if (fd != smb2->fd) {
    ...
    if (revents & POLLERR) {
        ...
        if (smb2->fd == -1 && smb2->next_addrinfo != NULL) {
            ...
            smb2_set_error(smb2,
                           "smb2_service: socket error "
                           "%s(%d).",
                           strerror(err),
                           err);
        } else {
            smb2_set_error(smb2,
                           "smb2_service: POLLERR, "
...
static void set_nonblocking(t_socket fd) {
#if defined(WIN32)
    unsigned long opt = 1;
...
static int set_tcp_sockopt(t_socket sockfd, int optname, int value) {
    /*
 return -1;
    int level;
#ifndef SOL_TCP
...
#endif
*/
    return setsockopt(sockfd, SOL_SOCKET, optname, (char*)&value, sizeof(value));
}

static int connect_async_ai(struct smb2_context* smb2, const struct addrinfo* ai, int* fd_out) {
    int                     family, fd;
    socklen_t               socksize;
...
int smb2_connect_async(struct smb2_context* smb2, const char* server, smb2_command_cb cb, void* private_data) {
    char *                 addr, *host, *port;
    int                    err;
    size_t                 addr_count = 0;
    const struct addrinfo* ai;
