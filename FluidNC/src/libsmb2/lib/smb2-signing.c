...
static int aes_cmac_shift_left(uint8_t data[AES128_KEY_LEN]) {
    int i    = 0;
    int cin  = 0;
    int cout = 0;
...
static void aes_cmac_xor(uint8_t data[AES128_KEY_LEN], const uint8_t value[AES128_KEY_LEN]) {
    int i = 0;
...
static void aes_cmac_sub_keys(uint8_t key[AES128_KEY_LEN], uint8_t sub_key1[AES128_KEY_LEN], uint8_t sub_key2[AES128_KEY_LEN]) {
...
void smb3_aes_cmac_128(uint8_t key[AES128_KEY_LEN], uint8_t* msg, uint64_t msg_len, uint8_t mac[AES128_KEY_LEN]) {
...
int smb2_calc_signature(struct smb2_context* smb2, uint8_t* signature, struct smb2_iovec* iov, int niov)
...
int smb2_pdu_add_signature(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    if (pdu->out.iov[0].len != SMB2_HEADER_SIZE) {
        smb2_set_error(smb2,
                       "First vector is not same size as smb2 "
                       "header");
        return -1;
    }
