 ...
 */
static int validate_utf8_cp(const char** utf8, uint16_t* ret) {
    int c = *(*utf8)++;
    int l, l_tmp;
    l = l_tmp = l1(c);
    uint32_t cp;
...
static int utf16_size(const uint16_t* utf16, int utf16_len) {
    int             length    = 0;
 ...
 */
const char* utf16_to_utf8(const uint16_t* utf16, int utf16_len) {
    int   utf8_len = 1;
    char *str, *tmp;
