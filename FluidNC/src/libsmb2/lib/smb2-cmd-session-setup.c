...
static int smb2_encode_session_setup_request(struct smb2_context* smb2, struct smb2_pdu* pdu, struct smb2_session_setup_request* req) {
    int                len;
    uint8_t*           buf;
    ...
    iov = smb2_add_iovector(smb2, &pdu->out, buf, req->security_buffer_length, free);
    return 0;
}
...
#define IOV_OFFSET (rep->security_buffer_offset - SMB2_HEADER_SIZE - (SMB2_SESSION_SETUP_REPLY_SIZE & 0xfffe))

int smb2_process_session_setup_fixed(struct smb2_context* smb2, struct smb2_pdu* pdu) {
    ...
    if (struct_size != SMB2_SESSION_SETUP_REPLY_SIZE || (struct_size & 0xfffe) != iov->len) {
        smb2_set_error(smb2,
                       "Unexpected size of Session Setup "
...
int smb2_process_session_setup_variable(struct smb2_context* smb2, struct smb2_pdu* pdu) {
