...
static int smb2_parse_args(struct smb2_context* smb2, const char* args) {
    while (args && *args != 0) {
        char *next, *value;
...
struct smb2_iovec* smb2_add_iovector(struct smb2_context* smb2, struct smb2_io_vectors* v, uint8_t* buf, int len, void (*free)(void*)) {
