...
void ntlmssp_destroy_context(struct auth_data* auth) {
    free(auth->ntlm_buf);
    free(auth->buf);
    free(auth);
}

struct auth_data* ntlmssp_init_context(
    const char* user, const char* password, const char* domain, const char* workstation, const char* client_challenge) {
...
static int encoder(const void* buffer, size_t size, void* ptr) {
...
static int ntlm_negotiate_message(struct smb2_context* smb2, struct auth_data* auth_data) {
    ...
    u32 = NTLMSSP_NEGOTIATE_128 | NTLMSSP_NEGOTIATE_EXTENDED_SESSIONSECURITY |
...
static int ntlm_challenge_message(struct auth_data* auth_data, unsigned char* buf, int len) {
...
static int ntlm_convert_password_hash(const char* password, unsigned char password_hash[16]) {
    int           i, hn, ln;
...
static int NTOWFv1(const char* password, unsigned char password_hash[16]) {
    MD4_CTX       ctx;
    struct utf16* utf16_password = NULL;
...
static int NTOWFv2(const char* user, const char* password, const char* domain, unsigned char ntlmv2_hash[16]) {
    int           i, len;
    char*         userdomain;
    ...
    smb2_hmac_md5((unsigned char*)utf16_userdomain->val, utf16_userdomain->len * 2, ntlm_hash, 16, ntlmv2_hash);
    free(userdomain);
 ...
 */
static int encode_temp(
    struct auth_data* auth_data, uint64_t t, char* client_challenge, char* server_challenge, char* server_name, int server_name_len) {
    unsigned char sign[8] = { 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
static int encode_ntlm_auth(struct smb2_context* smb2, time_t ti, struct auth_data* auth_data, char* server_challenge) {
    int                 ret = -1;
    ...
    /*
         ...
         */
    if (NTOWFv2(auth_data->user, auth_data->password, auth_data->domain, ResponseKeyNT) < 0) {
        goto finished;
    }
    ...
    if (encode_temp(auth_data, t, (char*)auth_data->client_challenge, server_challenge, server_name_buf, server_name_len) < 0) {
        return -1;
    }

    smb2_hmac_md5(&auth_data->buf[8], auth_data->len - 8, ResponseKeyNT, 16, NTProofStr);
...
encode:
    ...
    if (!anonymous) {
        ...
        smb2_hmac_md5(&lm_buf[0], 16, ResponseKeyNT, 16, LMStr);
    ...
    u32 = htole32((NTChallengeResponse_len << 16) | NTChallengeResponse_len);
    encoder(&u32, 4, auth_data);
    u32 = 0;
    encoder(&u32, 4, auth_data);
    ...
    if (!anonymous) {
        ...
        if (utf16_domain) {
            encoder(utf16_domain->val, utf16_domain->len * 2, auth_data);
        }
...
int ntlmssp_generate_blob(struct smb2_context* smb2,
                          time_t               t,
    ...
    if (input_buf == NULL) {
        ...
        if (ntlm_challenge_message(auth_data, input_buf, input_len) < 0) {
            return -1;
        }
        if (encode_ntlm_auth(smb2, t, auth_data, (char*)&auth_data->ntlm_buf[24]) < 0) {
            return -1;
        }
    }
