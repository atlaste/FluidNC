...
int smb2_decode_file_fs_volume_info(struct smb2_context* smb2, void* memctx, struct smb2_file_fs_volume_info* fs, struct smb2_iovec* vec) {
    uint64_t    t;
    const char* name;
    ...
    name             = utf16_to_utf8((uint16_t*)&vec->buf[18], fs->volume_label_length / 2);
...
int smb2_decode_file_fs_size_info(struct smb2_context* smb2, void* memctx, struct smb2_file_fs_size_info* fs, struct smb2_iovec* vec) {
    if (vec->len < 24) {
        return -1;
    }
...
int smb2_decode_file_fs_device_info(struct smb2_context* smb2, void* memctx, struct smb2_file_fs_device_info* fs, struct smb2_iovec* vec) {
    if (vec->len < 8) {
        return -1;
    }
...
int smb2_decode_file_fs_control_info(struct smb2_context* smb2, void* memctx, struct smb2_file_fs_control_info* fs, struct smb2_iovec* vec) {
    if (vec->len < 48) {
        return -1;
    }
...
int smb2_decode_file_fs_full_size_info(struct smb2_context* smb2, void* memctx, struct smb2_file_fs_full_size_info* fs, struct smb2_iovec* vec) {
    if (vec->len < 32) {
        return -1;
    }
...
int smb2_decode_file_fs_sector_size_info(struct smb2_context*                  smb2,
    ...
    smb2_get_uint32(vec, 4, &fs->physical_bytes_per_sector_for_atomicity);
    smb2_get_uint32(vec, 8, &fs->physical_bytes_per_sector_for_performance);
    smb2_get_uint32(vec, 12, &fs->file_system_effective_physical_bytes_per_sector_for_atomicity);
